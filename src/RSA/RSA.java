 ...
 * <p>Classe implementant l'algorithme de chiffrement Rivest Shamir Adleman (RSA) </p>
 */
public class RSA {
	...
	public RSA(BigInteger digest) throws Exception {
		//String message = "'Signature Numerique  Et Horrodatage'";
		System.out.println("Message a chiffrer = " + digest + "(Digest)");
		...
		BigInteger crypted =  rsa.encrypt(digest);
		System.out.println("\ncrypted = "+crypted);
	...
	/*
		...
		System.out.println("\nValeur ascii crypted = " + answer);
		
		
		//System.out.println("(decrypted == number) = "+(decrypted.compareTo(number)==0));
		
		BigInteger ans = RSAUtil.stringToNumber(answer);
		System.out.println("\nValeur Ascii decrypted = "+ans);
		*/
	}
}